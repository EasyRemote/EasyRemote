...
@device_action(name="robot.deploy_plan", description="Deploy mission plan on sandbox robot")
...
@device_action(name="robot.set_mode", description="Set robot mode")
def set_mode_action(mode: str = "auto", reason: str = "") -> Dict[str, Any]:
...
@device_action(name="robot.execute_action", description="Run one robot action")
def execute_action_action(
    action: str = "forward",
...
@device_action(name="robot.get_status", description="Get robot status snapshot")
def get_status_action() -> Dict[str, Any]:
    return {
        "status": "ok",
        "mode": _STATE["mode"],
        "battery": round(float(_STATE["battery"]), 2),
...
@device_action(name="robot.stream_telemetry", description="Stream robot telemetry frames")
def stream_telemetry_action(frame_count: int = 3, interval_ms: int = 80):
